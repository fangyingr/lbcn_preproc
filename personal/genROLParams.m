...
ROLparams.smooth = true;%false;   % true or false- whether to smooth signal before estimating ROL
ROLparams.smwin = 0.01;     % width of gaussian smoothing window (in s), if used
ROLparams.thr = 1.5;          % threshold used for determining rough onset (around which more accurate onset is estimated)- z-score relative to baseline
...
ROLparams.winlen = 0.03;    % length (s) of smaller windows where linear fit of data is computed
ROLparams.overlap = 0.028;   % length (s) of window overlap
...
ROLparams.bs_reps = 1000;    % number of bootstrap reps
...
ROLparams.noise_fields_trials= {'bad_epochs_HFO','bad_epochs_raw_HFspike','bad_epochs_raw_LFspike'}; % can combine any of the bad_epoch fields in data.trialinfo (will take union of selected fields)
ROLparams.noise_fields_timepts= {'bad_inds_HFO','bad_inds_raw_HFspike','bad_inds_raw_LFspike'}; % can combine any of the bad_epoch fields in data.trialinfo (will take union of selected fields)
