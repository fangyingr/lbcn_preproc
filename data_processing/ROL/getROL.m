function [ROL]= getROL(data,params)
%% INPUTS:
% data: epoched data
%       .wave: actual data
...
bs_rol_thr = nan(1,size(data_bc,1)); %bootstrapped onsets
bs_rol_fit = nan(1,size(data_bc,1)); %bootstrapped onsets

start_ind = find(data.time > params.minrol,1);
for ti = 1:size(data_bc,1)
    ...
    while (numwins < mindur_pt) && (ind < siglength)
        ...
        if (nanmean(data_bc(ti,temp_inds))>thr_val)
