function [pval, t_orig, clust_info, seed_state, est_alpha,timing,issig,time_events] = clusterPermutationStats(data,column,conds,stats_params,plot_params)
...
%% time window
% time_inds = find(data.time >= stats_params.twin(1) & data.time <= stats_params.twin(2));
time_inds = 1:length(data.time);
...
onset_indx= find(data.time>0);
if strcmp (datatype,'Band')
    snp=intersect(pvalsig,find(t_orig>0));  % positive
    snn=intersect(pvalsig,find(t_orig<0));  % negtive
    snp=intersect(snp,onset_indx);
    snn=intersect(snn,onset_indx);    
    diff_snp=[];
    if ~isempty(snp)
        x= diff(snp);
        i = find(diff(x));
        ...
        n=[1,n];
        nn=cumsum(n);
        fc=find(n>stats_params.mincluster*data.fsample,1);
        timing.start = data.time(snp(nn(fc-1)));
        timing.stop =data.time(snp(nn(fc)));
    ...
    if stats_params.isplot && ~isempty(pvalsig)
